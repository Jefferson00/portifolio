...
export function Modal(){
    const {isModalOpen, closeModal, selectedProject} = useContext(ProjectsContext)
    let cont = 0
        ...
        {selectedProject &&
            <div className={styles.modalContainer} id="modal">
                <div className={styles.modalContent}>
                    ...
                    <span className={styles.navButton} id="prev" onClick={()=>prevImage()}>
                        
                    </span>
                    <span className={styles.navButton} id="next" style={{
                        right:0,
                        margin: '0 1.5rem 0 0',
                    ...
                    <div className={styles.imagemContainer} id="imageContainer">
                        {selectedProject.gallery.map((image,index)=>{
                            return(
                                index == 0 ?
                                <img src={image.image} alt={selectedProject.title} key={index}/>
                                :
                                <img src={image.image} alt={selectedProject.title} key={index} className={styles.hiddenImage}/>
                            )
                        })}
                    </div>
                    <div className={`${styles.informationsContainer}`} id="information" >
                        <span className={styles.expandButton} onClick={handleInformations}/>
                        <strong>{selectedProject.title}</strong>
                        <p>{selectedProject.description}</p>
                        <p className={styles.title}>Tecnologias utilizadas:
                        ...
                        <p className={styles.title}>Repositório:
